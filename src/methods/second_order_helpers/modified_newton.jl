...
# that H + λI is positive definite. The algorithm that is implemented
# below is Algorithm 3.3 from Nocedal and Wright, "Numerical Optimization".

"""
    add_identity!(A::Matrix{T}, λ::T) where {T}

Add the scalar `λ` only to the diagonal of the matrix `A`, modifying `A` 
in place.

# Arguments

- `A::Matrix{T}`, matrix of values for which the diagonal will be edited.
...
"""
    add_identity_until_pd!(res::Matrix{T}; λ::T = T(0), β::T = T(1e-3),
        max_iterations::Int64 = 10) where {T}

Function that takes a matrix `res`, and tries to find a scalar `λ` such that
`res + λI` is positive definite. The matrix `res` is edited in-place, and
if a scalar is found, the upper triangular portion will be overwritten with
...
!!! warning
    ...
    if `res` was not symmetric, the method might appear to factorize the
    matrix correctly but this would be incorrect.
...
Return two values. 
1. First value is the value of `λ` for which the function terminated on. 
2. The second value is a boolean flag indicating whether a factorization
