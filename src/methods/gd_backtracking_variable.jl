...
# Purpose: Implementation of backtracking with an adjusted step size
# so it doesn't fail so often for the experiments

"""
    BacktrackingVariableGD{T} <: AbstractOptimizerData{T}
...
# Constructors
    ...
    BacktrackingVariableGD(::Type{T}; x0::Vector{T}, α::T, δ::T, ρ::T,
        line_search_max_iteration::Int64, threshold::T, max_iteration::Int64)
        where {T}

Returns a `struct` of type `BacktrackingVariableGD{T}` with all the field initialized to
...
function BacktrackingVariableGD(
    ::Type{T};
    x0::Vector{T},
    α::T,
    δ::T,
...
"""
    backtracking_variable_gd(optData::BacktrackingVariableGD{T},
...
where ``||\\cdot||_2`` is the L2-norm. We allow variable starting for the 
initial step size according to [Nocedal and Wright. "Numerical Optimization". 
    Springer New York, NY.](@cite nocedal2006Numerical) page 59. 
...
- `optData::BacktrackingVariableGD{T}`, the specification for the optimization method.
...
!!! warning
    ...
    a `grad` argument.
"""
function backtracking_variable_gd(
    optData::BacktrackingVariableGD{T},
    ...
    while (iter < optData.max_iterations) &&
        ...
        norm_squared_k = optData.grad_val_hist[iter]^2 
        backtracking_condition_satisfied = OptimizationMethods.backtracking!(x, 
            optData.iter_hist[iter], F, store.grad, norm_squared_k, 
        ...
        αk = (x[1] - optData.iter_hist[iter][1]) / (-store.grad[1])
