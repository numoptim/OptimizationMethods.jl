...
Given iterates ``\\lbrace x_0,\\ldots,x_k\\rbrace``, the iterate ``x_{k + 1}``
    ...
    to `optData.step_size_scaling * optData.step_size_function(T, k + 1)`.

The step size sequence generated by the step size function must satisfy the
    following two conditions.
```math
\\sum_{k = 0}^\\infty \\alpha_k = \\infty \\text{\\quad and \\quad} 
...
function diminishing_step_gd(
    ...
    while (iter < optData.max_iterations) &&
        ...
        # take a step
        step_size = optData.step_size_scaling * 
            optData.step_size_function(T, iter)
