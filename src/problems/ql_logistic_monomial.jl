...
# Purpose: Implement quasi-likelihood with a logistic link function and
# the monomial_plus_constant function

"""
    QLLogisticMonomial{T, S} <: AbstractDefaultQL{T, S}

Implements a quasi-likelihood objective with a logistic link function and the
    variance function `monomial_plus_constant`. If the design matrix and the
...
!!! note
    For this to be a valid variance function that is differentiable everywhere,
    ``p > .5`` and ``c > 0``.
...
Initialize the data structure for a quasi-likelihood estimation problem with
    a [logistic link function](@ref OptimizationMethods.logistic) and
    a [monomial plus constant variance function](@ref 
...
"""
mutable struct QLLogisticMonomial{T, S} <: AbstractDefaultQL{T, S}
    ...
    begin
        V(μ) = OptimizationMethods.monomial_plus_constant(μ, p, c)
        dV(μ) = OptimizationMethods.dmonomial_plus_constant(μ, p)
...
function QLLogisticMonomial(
    ...
    @assert nobs > 0 "Number of observations $(nobs) should be positive."

    @assert nvar > 0 "Number of variables $(nvar) should be positive."

    # initialize the meta data and counters
    meta = NLPModelMeta(
        nvar,
        name = "Quasi-likelihood with logistic link and monomial plus constant"*
        "variance",
        x0 = zeros(T, nvar)
    )
    counters = Counters()
    ...
    ϵ = T.((rand(Distributions.Arcsine(), nobs) .- .5)./(1/8))
...
function QLLogisticMonomial(
    ...
    @assert size(design, 2) == length(x0) "Number of columns in design matrix"*
...
"""
    PrecomputeQLLogisticMonomial{T} <: AbstractDefaultQLPrecompute{T}
...
# Constructor
    ...
    PrecomputeQLLogisticMonomial(progData::QLLogisticMonomial{T, S}) where {T, S}

Initializes the field values for the precompute data structure and returns 
    a `struct`.
"""
struct PrecomputeQLLogisticMonomial{T} <: AbstractDefaultQLPrecompute{T}
    obs_obs_t::Array{T, 3}
end
function PrecomputeQLLogisticMonomial(progData::QLLogisticMonomial{T, S}
    ) where {T, S}
    ...
    return PrecomputeQLLogisticMonomial{T}(obs_obs_t)
end

"""
    AllocateQLLogisticMonomial{T} <: AbstractDefaultQLAllocate{T}
...
# Constructors
    ...
    AllocateQLLogisticMonomial(progData::QLLogisticMonomial{T, S}) where {T, S}

Allocates memory for each of the field values and returns the struct.
"""
struct AllocateQLLogisticMonomial{T} <: AbstractDefaultQLAllocate{T}
...
function AllocateQLLogisticMonomial(progData::QLLogisticMonomial{T, S}
    ) where {T, S}
    ...
    return AllocateQLLogisticMonomial(
...
"""
    initialize(progData::QLLogisticMonomial{T, S}) where {T, S}

Creates a `PrecomputeQLLogisticMonomial` and `AllocateQLLogisticMonomial`
    struct, returning them in that order.
"""
function initialize(progData::QLLogisticMonomial{T, S}) where {T, S}
    precomp = PrecomputeQLLogisticMonomial(progData)
    store = AllocateQLLogisticMonomial(progData)
