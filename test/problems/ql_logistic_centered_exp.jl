...
@testset "Problem Struct -- QLLogisticCenteredExp" begin

    # set seed for reproducibility
    Random.seed!(1010)

    ####################################
    # Test Struct: QLLogisticCenteredExp{T, S}
    ####################################

    # Check if struct is defined 
    @test isdefined(OptimizationMethods, :QLLogisticCenteredExp)
    
    # Check supertype 
    @test supertype(OptimizationMethods.QLLogisticCenteredExp) == 
    ...
    let real_types = [Float16, Float32, Float64], nobs_default = 1000,
        nvar_default = 50, p_default = 2.0, c_default = 2.0 

        for real_type in real_types 
            # Check Assertion
            @test_throws AssertionError OptimizationMethods.QLLogisticCenteredExp(
            ...
            progData = OptimizationMethods.QLLogisticCenteredExp(real_type, nobs=nobs_default, 
                nvar=nvar_default, p = real_type(p_default), c = real_type(c_default))
